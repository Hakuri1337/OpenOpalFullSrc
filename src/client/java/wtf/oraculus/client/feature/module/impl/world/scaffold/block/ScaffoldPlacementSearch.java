package wtf.oraculus.client.feature.module.impl.world.scaffold.block;

import net.minecraft.block.BlockState;
import net.minecraft.util.math.BlockPos;
import net.minecraft.util.math.Direction;
import net.minecraft.util.math.Vec3d;
import wtf.oraculus.client.feature.module.impl.world.scaffold.state.ScaffoldPlacementCandidate;
...
public final class ScaffoldPlacementSearch {
    private ScaffoldPlacementSearch() {
    }

    public static ScaffoldPlacementTarget findShellTarget(final Vec3d eye, final int targetYLevel) {
        if (mc.world == null || mc.player == null) {
            return null;
        }

        final BlockPos belowFeet = BlockPos.ofFloored(eye.x, targetYLevel + 0.1D, eye.z);
        if (ScaffoldBlockUtil.hasSolidTop(belowFeet)) {
            return null;
        }

        ScaffoldPlacementTarget result = targetFromAirPosition(eye, belowFeet);
        if (result != null) {
            return result;
        }
    ...
    public static ScaffoldPlacementTarget findLegacyBfsTarget(
        ...
        final PriorityQueue<ScaffoldPlacementCandidate> queue = new PriorityQueue<>(
                Comparator.comparingDouble(candidate -> manhattan(candidate.pos(), origin))
        );
        final Set<BlockPos> visited = new HashSet<>();
        queue.offer(new ScaffoldPlacementCandidate(origin, null, 0));
        visited.add(origin);

        while (!queue.isEmpty()) {
            final ScaffoldPlacementCandidate candidate = queue.poll();
            for (final Direction direction : directions) {
                ...
                if (isValidLegacyBlock(neighbor, targetYLevel)) {
                    ...
                    if (mc.world.getBlockState(neighbor).isSolidSurface(mc.world, neighbor, mc.player, face)) {
                        return new ScaffoldPlacementTarget(neighbor, face);
                    }
                } else if (candidate.depth() < 3) {
                    queue.offer(new ScaffoldPlacementCandidate(neighbor, direction, candidate.depth() + 1));
                }
            }
        }
        return null;
    ...
    private static ScaffoldPlacementTarget targetFromAirPosition(final Vec3d eye, final BlockPos placePos) {
        if (!ScaffoldBlockUtil.isAir(placePos)) {
            return null;
        }
        ...
        for (final Direction sourceDirection : Direction.values()) {
            ...
            if (!ScaffoldBlockUtil.isSupportFace(supportPos, sourceDirection)) {
                continue;
            }
            final Vec3d delta = facePoint.subtract(eye);
            if (delta.lengthSquared() <= 20.25D
                    && delta.normalize().dotProduct(normal.normalize()) >= 0.0D) {
                return new ScaffoldPlacementTarget(supportPos.toImmutable(), sourceDirection.getOpposite());
            }
        }
        return null;
    }
    ...
    private static boolean isValidLegacyBlock(final BlockPos pos, final int targetYLevel) {
        ...
        return ScaffoldBlockUtil.isSearchSolid(state, pos)
