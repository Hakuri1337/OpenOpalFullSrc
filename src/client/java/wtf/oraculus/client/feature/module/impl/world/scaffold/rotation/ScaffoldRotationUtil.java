...
public final class ScaffoldRotationUtil {
    private ScaffoldRotationUtil() {
    }

    public static ScaffoldRotation smoothRotation(
            final ScaffoldRotation current,
            final ScaffoldRotation target,
            final double speed
    ) {
        ...
        for (int pass = 1; pass <= passes; pass++) {
            ...
            final ScaffoldRotation snapped = new ScaffoldRotation(targetYaw, targetPitch)
        ...
        return new ScaffoldRotation(targetYaw, targetPitch);
    }
    ...
    public static ScaffoldRotation rotationToBlock(final BlockPos blockPos, final float partialTicks) {
        if (mc.player == null) {
    ...
    public static ScaffoldRotation rotationFromVec(final Vec3d target) {
        if (mc.player == null) {
            return new ScaffoldRotation();
        }
    ...
    public static ScaffoldRotation rotationFromPoints(
        ...
        return new ScaffoldRotation(yaw, pitch);
    }

    public static ScaffoldRotation rotationFromDeltas(
            final double deltaX,
            final double deltaY,
            final double deltaZ
    ) {
        ...
        return new ScaffoldRotation(MathHelper.wrapDegrees(yaw), MathHelper.wrapDegrees(pitch));
    }
    ...
    public static Vec3d directionFromRotation(final ScaffoldRotation rotation) {
        return Vec3d.fromPolar(rotation.pitch(), rotation.yaw());
    }

    private static double addNoise(final double value) {
        return value + ScaffoldMathUtil.randomDouble(0.05D, 0.08D)
                * (ScaffoldMathUtil.randomDouble(0.0D, 1.0D) * 2.0D - 1.0D);
    }
