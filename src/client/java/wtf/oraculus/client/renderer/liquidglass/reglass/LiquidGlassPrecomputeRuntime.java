package wtf.oraculus.client.renderer.liquidglass.reglass;
...
public final class LiquidGlassPrecomputeRuntime {
    ...
    private GpuTexture sourceTexture;
    private GpuTextureView sourceView;
    ...
    private static final Identifier VS_ID = Identifier.of("oraculus", "core/liquid_glass_fullscreen");
    private static final Identifier BLUR_ID = Identifier.of("oraculus", "program/liquid_glass_blur");

    private LiquidGlassPrecomputeRuntime() {}

    private void ensureSourceTarget(int width, int height) {
        if (sourceTexture != null
                && sourceTexture.getWidth(0) == width
                && sourceTexture.getHeight(0) == height) {
            return;
        }
        if (sourceView != null) {
            sourceView.close();
        ...
        sourceTexture.setTextureFilter(FilterMode.LINEAR, false);
        sourceView = RenderSystem.getDevice().createTextureView(sourceTexture);
    }

    private void ensurePipelines() {
        if (blurPipeline == null) {
            blurPipeline = RenderPipeline.builder()
                    .withLocation(Identifier.of("oraculus", "pipeline/liquid_glass_blur"))
        ...
        if (samplerInfoUbo == null) {
            samplerInfoUbo = RenderSystem.getDevice().createBuffer(() -> "oraculus liquid glass SamplerInfo (pre)", 130, 16);
        }

        int blurConfigSize = 16 + (MAX_RADIUS + 1) * 16;
        if (blurConfigUboX == null) {
            blurConfigUboX = RenderSystem.getDevice().createBuffer(() -> "oraculus liquid glass BlurConfig X", 130, blurConfigSize);
        }
        if (blurConfigUboY == null) {
            blurConfigUboY = RenderSystem.getDevice().createBuffer(() -> "oraculus liquid glass BlurConfig Y", 130, blurConfigSize);
        }
    }
    ...
    private void ensureTempTarget(int w, int h) {
        if (blurTempTex == null || blurTempTex.getWidth(0) != w || blurTempTex.getHeight(0) != h) {
            ...
            blurTempTex = RenderSystem.getDevice().createTexture("oraculus liquid glass blur temp", 12, TextureFormat.RGBA8, w, h, 1, 1);
    ...
    private void ensureOutputForRadius(int w, int h, int radius) {
        ...
        if (tex == null || tex.getWidth(0) != w || tex.getHeight(0) != h) {
            ...
            GpuTexture newTex = RenderSystem.getDevice().createTexture("oraculus liquid glass blurred r=" + radius, 12, TextureFormat.RGBA8, w, h, 1, 1);
    ...
    public void run() {
        ...
        ce.copyTextureToTexture(main.getColorAttachment(), sourceTexture, 0, 0, 0, 0, 0, w, h);
        ...
        for (int k = 0; k < max; k++) {
            ...
            try (RenderPass pass = ce.createRenderPass(() -> "oraculus liquid glass blur X r=" + radius, blurTempView, OptionalInt.empty())) {
                pass.setPipeline(blurPipeline);
                RenderSystem.bindDefaultUniforms(pass);
                pass.setUniform("SamplerInfo", samplerInfoUbo);
                pass.setUniform("Config", blurConfigUboX);
                pass.bindSampler("DiffuseSampler", sourceView);
                pass.setVertexBuffer(0, quadVB);
                pass.setIndexBuffer(ib, it);
                pass.drawIndexed(0, 0, 6, 1);
            }

            try (RenderPass pass = ce.createRenderPass(() -> "oraculus liquid glass blur Y r=" + radius, blurredViewByRadius.get(radius), OptionalInt.empty())) {
                pass.setPipeline(blurPipeline);
                RenderSystem.bindDefaultUniforms(pass);
                pass.setUniform("SamplerInfo", samplerInfoUbo);
    ...
    public GpuTextureView getSourceView() {
        return sourceView;
    }
}
