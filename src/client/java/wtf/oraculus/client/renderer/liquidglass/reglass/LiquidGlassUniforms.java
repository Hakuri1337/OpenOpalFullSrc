...
import wtf.oraculus.client.renderer.liquidglass.reglass.gui.LiquidGlassGuiElementRenderState;
import wtf.oraculus.mixin.reglass.GuiRenderStateAccessor;
...
public final class LiquidGlassUniforms {
    ...
    private boolean loggedWidget;
    private boolean loggedComposite;
    ...
    private LiquidGlassUniforms() {
        samplerInfo = RenderSystem.getDevice().createBuffer(() -> "oraculus liquid glass SamplerInfo", 130, 16);
        ...
        customUniforms = RenderSystem.getDevice().createBuffer(() -> "oraculus liquid glass CustomUniforms", 130, customUniformsSize);

        int widgetInfoSize = 16 + MAX_WIDGETS * (16 * 12);
        widgetInfo = RenderSystem.getDevice().createBuffer(() -> "oraculus liquid glass WidgetInfo", 130, widgetInfoSize);
        ...
        bgConfig = RenderSystem.getDevice().createBuffer(() -> "oraculus liquid glass BgConfig", 130, bgConfigSize);
    }
    ...
    public void tryApplyBlur(DrawContext context) {
        ...
        int blurLayer = ((GuiRenderStateAccessor) state).oraculus$getBlurLayer();
    ...
    public void addWidget(LiquidGlassGuiElementRenderState element) {
        ...
        if (!loggedWidget) {
            loggedWidget = true;
            LOGGER.info("Oraculus ReGlass collected widget ({}, {})-({}, {}), radius={}, scale={}",
                    element.x1(), element.y1(), element.x2(), element.y2(), element.cornerRadius(),
                    MinecraftClient.getInstance().getWindow().getScaleFactor());
        }
    }

    public void logCompositeOnce() {
        if (loggedComposite) return;
        loggedComposite = true;
        MinecraftClient mc = MinecraftClient.getInstance();
        LOGGER.info("Oraculus ReGlass composite: widgets={}, framebuffer={}x{}",
                widgets.size(), mc.getFramebuffer().textureWidth, mc.getFramebuffer().textureHeight);
    }
