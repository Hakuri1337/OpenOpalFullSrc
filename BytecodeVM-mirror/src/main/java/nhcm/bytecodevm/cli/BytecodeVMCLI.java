...
import nhcm.bytecodevm.tools.GeneratedJarVerifier;
...
public final class BytecodeVMCLI implements Callable<Integer>
{
    ...
    static final class ProtectCommand extends ConfigCommand
    {
        @CommandLine.Option(
                names = "--verify-output",
                description = "Verify every generated class with ASM before accepting the output.")
        ...
        public Integer call()
        {
            ...
            try
            {
                ...
                if (verifyOutput)
                {
                    GeneratedJarVerifier.verify(config.outputFile);
                    report = report.withOutputVerified(true);
                }
            }
