...
class AppConfig {
  static defaults(configPath) {
    ...
    return {
      ...
      AllowedClientVersions: ['b5'], AllowedBuildIds: ['b5-free', 'b5-beta'],
...
class AuthService {
  ...
  register(request, remoteIp, requestId) {
    ...
    if (!AppConfig.allowedClient(this.config, edition, clientVersion, buildId))
      return error('CLIENT_VERSION_BLOCKED', '当前客户端版本不受支持，请更新客户端');
    ...
    if (edition === 'BETA') return { Ok: true, Message: '注册成功，当前账号为 Free，Beta 需要管理员开通', Account: accountView(user) };
    return this.issueSession(user, hwidHash, 'FREE', clientVersion, buildId, remoteIp, requestId);
  }
  ...
  login(request, remoteIp, requestId) {
    ...
    return this.issueSession(user, hwidHash, edition, clientVersion, buildId, remoteIp, requestId);
  }
  ...
  refresh(request, remoteIp, requestId) {
    ...
    return successSession(user, access, refresh, accessExpiresAt, refreshExpiresAt);
  }
  ...
  heartbeat(accessToken, remoteIp, requestId) {
    ...
    if (!AppConfig.allowedClient(this.config, session.clientEdition, session.clientVersion, session.buildId))
      return error('CLIENT_VERSION_BLOCKED', '当前客户端版本已停止支持，请更新客户端');
    ...
    return { Ok: true, Message: 'ok', Account: accountView(user) };
  }
  ...
  issueSession(user, hwidHash, edition, clientVersion, buildId, remoteIp, requestId) {
    ...
    this.store.mutate(data => data.sessions.push({
      ...
      clientEdition: edition, clientVersion, buildId, accessExpiresAtUtc: accessExpiresAt, refreshExpiresAtUtc: refreshExpiresAt,
  ...
  validateAccount(user, edition) {
    ...
    if (edition === 'BETA') {
      ...
      if (!user.betaExpiresAtUtc || user.betaExpiresAtUtc <= now()) return error('LICENSE_EXPIRED', 'Beta 授权已到期');
    }
    return null;
  }
...
function accountView(user) {
  return { id: user.id, username: user.username, role: user.role, tier: user.tier, status: user.status,
    betaExpiresAt: user.betaExpiresAtUtc, hwidBound: Boolean(user.hwidHash), hwidQuality: user.hwidQuality || '',
    passwordChangedAt: user.passwordChangedAtUtc, hwidChangedAt: user.hwidChangedAtUtc,
    forcePasswordChange: Boolean(user.forcePasswordChange) };
}
function successSession(user, access, refresh, accessExpiresAt, refreshExpiresAt) {
  return { Ok: true, Message: '登录成功', AccessToken: access, RefreshToken: refresh,
    AccessExpiresAt: accessExpiresAt, RefreshExpiresAt: refreshExpiresAt, Account: accountView(user) };
}
...
function userPage(user, csrf, note) {
...
function adminPage(session, listing, note) {
...
function startServer(configPath) {
  const config = AppConfig.load(configPath); const auth = new AuthService(config); let active = 0;
  const handler = async (request, response) => {
    const requestId = id(); active += 1;
    try {
      const secure = Boolean(request.socket.encrypted); securityHeaders(response, requestId, secure);
      if (active > MAX_CONCURRENT_REQUESTS) return writeJson(response, 503, { ok: false, error: 'SERVER_BUSY', message: '服务器繁忙，请稍后重试', requestId });
      if (!hostAllowed(config, request)) return writeText(response, 404, 'Not Found');
      if (config.RequireHttps && !secure) return writeJson(response, 426, { ok: false, error: 'HTTPS_REQUIRED', message: '必须使用 HTTPS', requestId });
      ...
      if (route.startsWith('/api/v1/')) return await handleApi(auth, request, response, method, route, remoteIp, requestId);
  ...
  const servers = [];
  if (config.EnableHttp) { const server = http.createServer(handler); server.listen(config.HttpPort, config.ListenHost); servers.push(server); }
  if (config.TlsCertificatePath || config.TlsPrivateKeyPath) {
    ...
    const server = https.createServer({ cert: fs.readFileSync(config.TlsCertificatePath), key: fs.readFileSync(config.TlsPrivateKeyPath), minVersion: 'TLSv1.2' }, handler);
...
async function handleApi(auth, request, response, method, route, remoteIp, requestId) {
  if (method !== 'POST' && !(method === 'GET' && route === '/api/v1/auth/status')) throw new RequestError(405, 'METHOD_NOT_ALLOWED', '请求方法不受支持');
  let payload = {};
  if (method === 'POST') {
...
async function handleUser(auth, config, request, response, method, route, url, remoteIp, requestId) {
  ...
  if (method === 'GET' && route === '/user') return writeHtml(response, userPage(auth.getUser(session.userId), session.csrfToken, url.searchParams.get('message')));
...
async function handleAdmin(auth, config, request, response, method, route, url, remoteIp, requestId) {
  ...
  if (method === 'GET' && route === '/admin') {
    ...
    return writeHtml(response, adminPage(session, listing, url.searchParams.get('message')));
  }
...
function selfTest() {
  ...
  try {
    ...
    const superPage = adminPage(rootSession, auth.listUsers('bulk_', 3), null);
    ...
    const supportPage = adminPage(supportSession, auth.listUsers('support_admin', 1), null);
    ...
    const registered = auth.register({ username: 'selftest_user', password: 'SelfTestPassword!2026', deviceFingerprint: fingerprint, hwidVersion: 'v1', hwidQuality: 'STRONG', edition: 'FREE', clientVersion: 'b5', buildId: 'b5-free' }, '127.0.0.1', 'self-register');
    ...
    const refreshed = auth.refresh({ refreshToken: registered.RefreshToken, deviceFingerprint: fingerprint, edition: 'FREE', clientVersion: 'b5', buildId: 'b5-free' }, '127.0.0.1', 'self-refresh');
    if (!refreshed.Ok || refreshed.RefreshToken === registered.RefreshToken) throw new Error('refresh self-test failed');
    const replay = auth.refresh({ refreshToken: registered.RefreshToken, deviceFingerprint: fingerprint, edition: 'FREE', clientVersion: 'b5', buildId: 'b5-free' }, '127.0.0.1', 'self-replay');
    ...
    const beta = auth.login({ username: 'selftest_user', password: 'SelfTestPassword!2026', deviceFingerprint: fingerprint, hwidVersion: 'v1', hwidQuality: 'STRONG', edition: 'BETA', clientVersion: 'b5', buildId: 'b5-beta' }, '127.0.0.1', 'self-beta');
...
module.exports = { AppConfig, AuthService, JsonStore };
